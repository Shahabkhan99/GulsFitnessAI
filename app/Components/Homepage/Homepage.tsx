...
const promptSend = async (promptText: string, type: "workout" | "diet") => {
  try {
    ...
    if (data.error) {
      console.error("AI failed:", data.details);
      const errorMsg = "Sorry, I ran into an error. Please try again later.";
      type === "workout" ? setAiWoResp(errorMsg) : setAiDietResp(errorMsg);
      return;
    }

    const notice =
      data.source === "huggingface"
        ? "(Generated by free fallback AI)"
        : data.source === "cache"
        ? "(From cached result)"
        : "";

    if (type === "workout") setAiWoResp(`${data.text}\n${notice}`);
    if (type === "diet") setAiDietResp(`${data.text}\n${notice}`);
  } catch (err) {
    console.error(err);
    const errorMsg = "Sorry, I ran into an error. Please try again later.";
    type === "workout" ? setAiWoResp(errorMsg) : setAiDietResp(errorMsg);
